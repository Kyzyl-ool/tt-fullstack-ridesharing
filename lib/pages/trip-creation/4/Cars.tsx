...
const Cars = () => {
    ...
    return (
        <BaseLayer type={'SECONDARY'} header={'Выберите автомобиль'}>
            {cars && cars.map(value => (
                <CarCard key={value.id} name={value.name} number={value.number} color={value.color} text={value.text}
                         id={value.id} onDelete={onDelete} onNewChanges={onNewChanges}/>)
            )}
        </BaseLayer>
    )
};
