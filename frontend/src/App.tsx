            ...
            <Switch>
              {authorized && (
                  <MainContainer onLogout={() => setAuthorized(false)} show={authorized} onClick={() => setDrawerOpened(!drawerOpened)}>
                    ...
                    <Route exact path="/organizations" component={() => <OrganizationPage organizations={organizations} />}/>
                    {
                      organizations && organizations.map((value, index) => (
                        <Route key={index} exact path={`/organizations/${value.id}`}
                               component={() => <OrganizationCard name={value.name} amountOfPeople={value.users.length}
